...
- FIX: `update_document` no longer forces `e_invoice: true` and `ei_data.payment_method: "MP05"` on every edit. Both are echoed from the document, so an unrelated change stops rewriting what the e-invoice XML declares; `ei_data` is sent only when the document really is an e-invoice, as the spec requires.
