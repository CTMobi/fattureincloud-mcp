...
- FIX: `create_invoice`, `create_credit_note` and `create_proforma` take the payment method from the client registry (`default_payment_method`), the same one the FattureInCloud panel uses, instead of declaring MP05 (cash) in the XML of every document regardless of how it is actually collected. MP05 remains the fallback when the client has none.
