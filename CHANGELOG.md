...
- FIX: `update_document` no longer recomputes the installment amount from the lines when the caller did not touch them. The local total models the lines only, while FIC derives the document total from ritenuta d'acconto, cassa previdenziale, rivalsa INPS, marca da bollo and split payment as well — so on a professional invoice storing 1020,00, an edit to the subject alone rewrote the installment to 1220,00 and reported success, and the same mismatch refused unrelated edits claiming a total had changed. An edit that does touch the lines is refused outright when the document carries any of those rates, since the amount FIC would compute cannot be reproduced here (the taxable bases they apply to are not part of that test: FIC values them independently of the rate). `duplicate_invoice` and `convert_proforma_to_invoice` copy the modifiers, so a copy is no longer due a different amount than the original, and every creation path now sends `options.fix_payments`, which has FIC size the installment from the document it actually builds instead of from the lines alone.
