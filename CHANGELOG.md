...
- FIX: `scripts/validate.sh` reads the bundle size with the GNU `stat -c '%s'` first and the BSD `-f '%z'` as fallback (the previous order aborted the script on Linux, where `stat -f` inspects the filesystem), warns instead of reporting 0 MB when neither form works, and its tool check now extracts the packed `.mcpb` and compares the manifest against the tools that bundle actually serves — which also catches a dependency that breaks the entry point, the way `mcp` 2.x did.
