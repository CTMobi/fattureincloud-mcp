...
- CHANGE: `list_invoices` and `list_received_documents` return `{count, pages, truncated, documents}` instead of a bare array. They still read one page of 100, but a year that does not fit now says so instead of looking complete. `check_numeration` reuses the shared pager.
- NEW: `vat_id` on a document line, as the way out of an ambiguous rate: several VAT types can share a percentage while differing in *natura* (the 0% ones end up as N1…N7 in the e-invoice XML), so `vat_rate` alone is refused there with the candidates listed rather than resolved to the lowest id.
